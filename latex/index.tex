...
The primary use case for 🌊 sailfish-\/pool is within \href{https://zenroo.org}{\texttt{ Zenroom}}, our small virtual machine (VM) designed for cryptographic operations where Lua is embedded.
...
The header includes its own implementation and uses internal linkage, so it is safe to include from multiple translation units in the same program.\hypertarget{index_autotoc_md3}{}\doxysubsubsection{\texorpdfstring{High Level API}{High Level API}}\label{index_autotoc_md3}
The main entry point is the \href{https://dyne.org/sailfish-pool/group__sfpool.html}{\texttt{ 🌊 High Level API documented here}} and constituted by init/teardown functions initalizing an sfpool context and malloc/free/realloc functions for common memory operations. Also a function to verify if a pointer is contained in the pool and one to report status.\hypertarget{index_autotoc_md4}{}\doxysubsubsection{\texorpdfstring{Utilities API}{Utilities API}}\label{index_autotoc_md4}
Some internal functions are exposed as the \href{https://dyne.org/sailfish-pool/group__sfutil.html}{\texttt{ 🌊 utilities API documented here}} and they may be useful also to host applications\+: fast and portable memory zeroing, memory alignment and the internal portable implementations for secure allocation and free.\hypertarget{index_autotoc_md5}{}\doxysubsection{\texorpdfstring{Quality Assurance}{Quality Assurance}}\label{index_autotoc_md5}
There is a makefile target in this repository running tests with the address sanitizer\+: just type {\ttfamily make check}.

For the standalone stress test you can also build and run it directly\+: \begin{DoxyVerb}gcc -Wall -Wextra -g -fsanitize=address -fsanitize=undefined -I. sfpool_test.c -o sfpool_test
time ./sfpool_test
\end{DoxyVerb}
...
The most useful local commands are {\ttfamily make sfpool\+\_\+test}, {\ttfamily make check}, and {\ttfamily make check-\/lua}.

Additional tests are available\+: {\ttfamily make wasm} builds and runs the test as a WASM binary when {\ttfamily EMSDK} is available and pointing to an Emscripten installation.

Then the {\ttfamily make check-\/lua} target downloads the latest stable Lua codebase and compiles it applying sfpool as its main memory allocator, then runs the Lua test suite.

All tests are constantly verified in \href{https://github.com/dyne/sailfish-pool/actions}{\texttt{ continuous integration by Github actions}}.\hypertarget{index_autotoc_md6}{}\doxysubsection{\texorpdfstring{License}{License}}\label{index_autotoc_md6}
Copyright (C) 2025 Dyne.\+org foundation

Designed and written by \href{https://jaromil.dyne.org}{\texttt{ Jaromil}}.
